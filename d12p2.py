...
def unfold_condition(condition: tuple[str, list[int]]
                     ) -> tuple[str, list[int]]:
    unfolded_spring_row = '?'.join([condition[0]] * 5)
    unfolded_damage_groups = condition[1] * 5
    return (unfolded_spring_row, unfolded_damage_groups)
...
def d12p2(data = None):
    ...
    number_of_arrangements = 0
    spring_conditions = map(unfold_condition, parse_input(data))
